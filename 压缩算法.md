...
该文件不是第二份作者源，也不替代 Provider 的 .cardtpl。它是由全部原始 Provider 模板生成的只读
压缩 DAG 快照，便于直接检查压缩结果：
...
例如查看 Battery 时，先在 templates 找到 BatteryOverview...@1 的 rootComponentId 和
rootContentDigest，再沿着 nodes[摘要去掉 sha256: 前缀].childNodeIds 展开即可看到压缩后的树。
多个 Battery 模板具有相同的语义名和内容摘要，就表示它们复用了同一个组件子树。
