...
本分支的正式压缩产物是 Provider 下的组件文件：

~~~text
resources/source/providers/<provider>/components/*.cardtpl
~~~

原模板入口仍保留在 `templates/*.cardtpl`，并通过 `UseComponent("语义组件名@版本")` 引用组件文件。
编译器在绑定与结构校验前展开引用，因此 query 仍按原模板 ID 检索，运行时不读取压缩 DAG JSON。

离线检查时可以额外生成只读 DAG 快照：

- templates：保留每个模板的 ID、数据契约、参数和变体；每个变体有可读 rootComponentId，以及用于
  校验和节点索引的 rootContentDigest；
- nodes：每个结构唯一的节点只保存一次，childNodeIds 表达复用关系；
- metrics：记录压缩前节点出现次数、压缩后唯一节点数与共享引用数。

例如查看 Battery 时，先看三个薄入口共同引用的
`components/battery-overview-full-status-summary.cardtpl`；它就是可维护的完整组件内容。JSON 中的
rootComponentId、rootContentDigest 与 nodes 只用于复核全局结构共享。
