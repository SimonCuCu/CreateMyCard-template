...
- templates：保留每个模板的 ID、数据契约、参数和变体；每个变体以 rootNodeId 指向节点表；
- nodes：每个结构唯一的节点只保存一次，childNodeIds 表达复用关系；
- metrics：记录压缩前节点出现次数、压缩后唯一节点数与共享引用数。

例如查看 Battery 时，先在 templates 找到 BatteryOverview...@1 的 rootNodeId，再沿着
nodes[rootNodeId].childNodeIds 展开即可看到压缩后的树。多个 Battery 模板指向同一个节点 ID，
就表示它们复用了同一个组件子树。
